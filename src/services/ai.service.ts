...
export class AIService {
    static async analyzeLogistics(extractedText: string) {
    ...
    private static async analyzeWithClaude(text: string) {
    ...
    private static async analyzeWithGemini(text: string) {
    ...
    static async chatWithAgent(question: string, contextData: any) {
        const apiKey = (process.env.GEMINI_API_KEY || '').trim();
        const url = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${apiKey}`;

        // Construir un contexto reducido para no exceder los tokens (solo la info clave)
        const simplifiedContext = contextData.map((r: any) => ({
            unidad: r.file_name,
            fecha: r.created_at,
            indicadores: r.analysis_data?.indicadores || null
        })).filter((r: any) => r.indicadores !== null);
